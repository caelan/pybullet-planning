...
def set_camera_poses(args, meshes):
    ...
    for theta in np.arange(0, 2 * np.pi, 2 * np.pi / 10):
        ...
        view_matrix = p.computeViewMatrix(cameraEyePosition=[center[0] + x, center[1] + y, center[2] + z],
                                    cameraTargetPosition=center,
                                    cameraUpVector=[0, 0, 1])
        view_matrix = np.transpose(np.array(view_matrix).reshape(4, 4))
        cam_poses.append(view_matrix)
...
def check_constraint(action, meshes, delta, dist_thresh=0.0):
...
if __name__ == "__main__":
    ...
    expname = args.path.split('/')[-2]
    savedir = os.path.join(args.log_dir, expname)
    ...
    floor = p.loadURDF('models/table/table.urdf', globalScaling=2.0)
    ...
    for idx, action in enumerate(tqdm(actions)):
        ...
        # p.stepSimulation()
        # time.sleep(10/240)  # only needed if using p.GUI 

    # for debug: visualize the collision free actions:
    # breakpoint()
    for idx, action in enumerate(tqdm(collision_free_actions)):
        # move the object
        delta = [action[1], action[2], 0]
        p.resetBasePositionAndOrientation(action[0], delta, [0, 0, 0, 1])
        # get the constraints - collision etc...
        p.stepSimulation()
        # time.sleep(30/240)  # only needed if using p.GUI 
        
    # save the collision free actions 
    collision_free_actions = np.array(collision_free_actions)
    action_path = os.path.join(savedir, "actions.npy")
    np.save(action_path, collision_free_actions)
    with open(os.path.join(savedir, "mesh_pathes.json"), 'w') as f:
        json.dump(mesh_paths, f)
    np.save(os.path.join(savedir, "render_poses.npy"), cam_poses)    
    np.save(os.path.join(savedir, "center_of_bbox.npy"), center)
